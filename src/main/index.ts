...
function createSession(shellKey: string, cols: number, rows: number, senderId: number): string {
  const def = SHELLS[shellKey] ?? SHELLS.powershell
  const id = `pty-${++sessionSeq}`

  const proc = pty.spawn(def.path, def.args, {
    name: 'xterm-256color',
    cols,
    rows,
    cwd: os.homedir(),
...
function registerPtyHandlers(): void {
  ipcMain.handle('pty:create', (event, args: { shell: string; cols: number; rows: number }) => {
    return createSession(args.shell, args.cols, args.rows, event.sender.id)
  })

  ipcMain.on('pty:write', (_event, args: { id: string; data: string }) => {
    sessions.get(args.id)?.write(args.data)
  })
